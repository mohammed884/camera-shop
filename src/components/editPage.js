...
export default function EditPage({match}) {
    ...
    useEffect(() =>
    {
        ...
        fetch('http://localhost:8000/user', {
          ...
          .then((res) => res.json())
          .then(res => {
              if (!res.user.isAdmin) history.push('/home') 
          })
    ...
    const handleSubmit = (e) =>
    {
        ...
        axios.put(`http://localhost:8000/edit/product/${match.params.id}` , data)
        .then(res => {if (res.status == 201) return(setMsg(res.data)) })
        .catch(err => console.log(err));
        console.log(msg)
        var formData = new FormData();

        formData.append('cover' , cover)
        if (typeof formData.getAll('cover') !== 'string')
        {
    ...
    {
        e.preventDefault()
        axios.delete(`http://localhost:8000/delete/product/${match.params.id}`)
        .then(res => {if (res.status === 201) return history.push('/products')})
        .catch(err => console.log(err))
    }
    return (
        <div>
                 ...
                 <form encType="multipart/form-data">
                     ...
                     <div>
                             <div className="sizing">
                                 ...
                                 <input onFocus={inputFoucs} onBlur={inputBlur} type="text" name="title" id="title" onChange={(e) => setTitle(title = e.target.value)}/>
                             </div>

                             <div className="sizing">
                                 <label htmlFor="qty">Qty</label>
                                 <input onFocus={inputFoucs} onBlur={inputBlur} type="number" name="qty" id="qty"  onChange={(e) => setQty(qty = e.target.value)}/>
                             </div> 

                             <div className="sizing">
                                 <label htmlFor="price">Price</label>
                                 <input onFocus={inputFoucs} onBlur={inputBlur} type="number" name="price" id="price"  onChange={(e) => setPrice(price = e.target.value)}/> 
                             </div>   

                             <div className="sizing">
                                 <label htmlFor="desk">Description</label>
                                 <textarea name="desk" id="desk"  onChange={(e) => setDesk(desk = e.target.value)}></textarea>
                             </div>  
                             ...
                             <div className="sizing">
                                 ...
                                 <label htmlFor="" onClick={() => fileInput.current.click()} style={{cursor:'pointer'}}>Cover {!cover ? product.cover : cover.name}</label>
                             </div>

                            <div><button onClick={handleSubmit}>Edit</button>   <button onClick={deleteProduct}>Delete</button></div>
